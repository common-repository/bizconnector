...
class bizconnector_Comment {
  ...
  function bizconnector_Comment($wp_comment = null) {
    if ($wp_comment) {
  ...
  function import_wp_object($wp_comment) {
    ...
    if (!empty($wp_comment->user_id)) {
      $this->author = new bizconnector_Author($wp_comment->user_id);
    } else {
      unset($this->author);
    }
  }
  ...
  function comment_id_not_found() {
    global $bizconnector;
    $bizconnector->error("Post ID '{$_REQUEST['post_id']}' not found.");
  }
  
  function comment_closed() {
    global $bizconnector;
    $bizconnector->error("Post is closed for comments.");
  }
  
  function comment_on_draft() {
    global $bizconnector;
    $bizconnector->error("You cannot comment on unpublished posts.");
  }
  
  function comment_post_redirect() {
    global $comment, $bizconnector;
    $status = ($comment->comment_approved) ? 'ok' : 'pending';
    $new_comment = new bizconnector_Comment($comment);
    $bizconnector->response->respond($new_comment, $status);
  }
