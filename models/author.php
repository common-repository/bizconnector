...
class bizconnector_Author {
  ...
  // Note:
  //   bizconnector_Author objects can include additional values by using the
  //   author_meta query var.
  
  function bizconnector_Author($id = null) {
    if ($id) {
      $this->id = (int) $id;
    } else {
  ...
  function set_author_meta() {
    global $bizconnector;
    if (!$bizconnector->query->author_meta) {
      return;
    }
    ...
    $vars = explode(',', $bizconnector->query->author_meta);
