...
class bizconnector_Query {
  ...
  function __construct() {
    // Register bizconnector query vars
  ...
  function get_legacy_controller($json) {
    ...
    if ($json == 'submit_comment') {
      if ($bizconnector->controller_is_active('respond')) {
        return 'respond';
      } else {
        $bizconnector->error("The 'submit_comment' method has been removed from the Core controller. To use this method you must enable the Respond controller from WP Admin > Settings > bizconnector.");
      }
    } else if ($json == 'create_post') {
      if ($bizconnector->controller_is_active('posts')) {
        return 'posts';
      } else {
        $bizconnector->error("The 'create_post' method has been removed from the Core controller. To use this method you must enable the Posts controller from WP Admin > Settings > bizconnector.");
      }
    } else {
      return 'core';
    }
  ...
  function get_method($controller) {
    ...
    if (empty($method)) {
      ...
      return false;
    } else if (method_exists("bizconnector_{$controller}_Controller", $method)) {
      ...
      if (is_search()) {
        ...
        if (empty($_GET['json'])) {
          $bizconnector->error("Uknown method '$method'.");
        }
