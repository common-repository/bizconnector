...
class bizconnector_Introspector {
  ...
  public function get_posts($query = false, $wp_posts = false) {
    ...
    while (have_posts()) {
      ...
      if ($wp_posts) {
        ...
        $new_post = new bizconnector_Post($post);
      }
      $output[] = $new_post;
    }
    return $output;
  ...
  public function get_current_post() {
    global $bizconnector;
    extract($bizconnector->query->get(array('id', 'slug', 'post_id', 'post_slug')));
    if ($id || $post_id) {
      if (!$id) {
        $id = $post_id;
      }
  ...
  public function get_current_category() {
    global $bizconnector;
    extract($bizconnector->query->get(array('id', 'slug', 'category_id', 'category_slug')));
    if ($id || $category_id) {
      if (!$id) {
        $id = $category_id;
      }
  ...
  public function get_current_tag() {
    global $bizconnector;
    extract($bizconnector->query->get(array('id', 'slug', 'tag_id', 'tag_slug')));
    if ($id || $tag_id) {
      if (!$id) {
        $id = $tag_id;
      }
  ...
  public function get_current_author() {
    global $bizconnector;
    extract($bizconnector->query->get(array('id', 'slug', 'author_id', 'author_slug')));
    if ($id || $author_id) {
      if (!$id) {
        $id = $author_id;
      }
  ...
  public function get_author_by_id($id) {
    ...
    return new bizconnector_Author($id);
  }
  ...
  public function get_comments($post_id) {
    ...
    foreach ($wp_comments as $wp_comment) {
      $comments[] = new bizconnector_Comment($wp_comment);
    }
    return $comments;
  }
  ...
  public function get_attachments($post_id) {
    ...
    if (!empty($wp_attachments)) {
      foreach ($wp_attachments as $wp_attachment) {
        $attachments[] = new bizconnector_Attachment($wp_attachment);
      }
    }
    return $attachments;
  }
  ...
  public function get_attachment($attachment_id) {
    ...
    return new bizconnector_Attachment($wp_attachment);
  }
  ...
  public function attach_child_posts(&$post) {
    ...
    foreach ($wp_children as $wp_post) {
      $new_post = new bizconnector_Post($wp_post);
  ...
  protected function get_category_object($wp_category) {
    ...
    return new bizconnector_Category($wp_category);
  }
  ...
  protected function get_tag_object($wp_tag) {
    ...
    return new bizconnector_Tag($wp_tag);
  }
  ...
  protected function set_posts_query($query = false) {
    ...
    if ($bizconnector->query->page) {
      $query['paged'] = $bizconnector->query->page;
    }
    
    if ($bizconnector->query->count) {
      $query['posts_per_page'] = $bizconnector->query->count;
    }
    
    if ($bizconnector->query->post_type) {
      $query['post_type'] = $bizconnector->query->post_type;
    }
    
    if (!empty($query)) {
      query_posts($query);
      do_action('bizconnector_query', $wp_query);
    }
  }
