...
class bizconnector_Response {
  
  function setup() {
    global $bizconnector;
    $this->include_values = array();
    $this->exclude_values = array();
    if ($bizconnector->query->include) {
      $this->include_values = explode(',', $bizconnector->query->include);
    }
    // Props to ikesyo for submitting a fix!
    if ($bizconnector->query->exclude) {
      $this->exclude_values = explode(',', $bizconnector->query->exclude);
  ...
  function get_json($data, $status = 'ok') {
    ...
    $data = apply_filters('bizconnector_encode', $data);
    ...
    if (function_exists('json_encode')) {
      ...
      if (version_compare(PHP_VERSION, '5.3') < 0) {
        ...
        if ($bizconnector->query->json_encode_options) {
          $json_encode_options = $bizconnector->query->json_encode_options;
        }
    ...
    if ($bizconnector->query->json_unescaped_unicode) {
  ...
  function respond($result, $status = 'ok') {
    ...
    if ($bizconnector->query->dev || !empty($_REQUEST['dev'])) {
    ...
    } else if ($bizconnector->query->redirect) {
      $url = $this->add_status_query_var($bizconnector->query->redirect, $status);
      wp_redirect($url);
    } else if ($bizconnector->query->callback) {
      // Run a JSONP-style callback with the result
      $this->callback($bizconnector->query->callback, $json);
    } else {
      // Output the result
      $this->output($json);
    }
