...
class bizconnector {
  ...
  function __construct() {
    $this->query = new bizconnector_Query();
    $this->introspector = new bizconnector_Introspector();
    $this->response = new bizconnector_Response();
    add_action('template_redirect', array(&$this, 'template_redirect'));
    add_action('admin_menu', array(&$this, 'admin_menu'));
    add_action('update_option_bizconnector_base', array(&$this, 'flush_rewrite_rules'));
    add_action('pre_update_option_bizconnector_controllers', array(&$this, 'update_controllers'));
  }
  ...
  function template_redirect() {
    ...
    $enabled_controllers = explode(',', get_option('bizconnector_controllers', 'core'));
    ...
    if ($controller) {
      ...
      if ($method) {
        ...
        do_action("bizconnector", $controller, $method);
        do_action("bizconnector-{$controller}-$method");
  ...
  function admin_menu() {
    add_options_page('bizconnector Settings', 'bizconnector', 'manage_options', 'bizconnector', array(&$this, 'admin_options'));
  }
  
  function admin_options() {
  	include('bizc_admin.php'); 
  }
  
  function admin_options_json_api() {
    ...
    if (!empty($_REQUEST['_wpnonce']) && wp_verify_nonce($_REQUEST['_wpnonce'], "update-options")) {
      if ((!empty($_REQUEST['action']) || !empty($_REQUEST['action2'])) &&
        ...
        $this->save_option('bizconnector_controllers', implode(',', $active_controllers));
      }
      if (isset($_REQUEST['bizconnector_base'])) {
        $this->save_option('bizconnector_base', $_REQUEST['bizconnector_base']);
      }
    }
...
<div class="wrap">
  ...
  <h2>bizconnector Settings</h2>
  <form action="options-general.php?page=bizconnector" method="post">
          ...
          <tr class="<?php echo ($active ? 'active' : 'inactive'); ?>">
            ...
            <td class="plugin-title">
              ...
              <div class="row-actions-visible">
                ...
                if ($active) {
                  echo '<a href="' . wp_nonce_url('options-general.php?page=bizconnector&amp;action=deactivate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Deactivate this controller') . '" class="edit">' . __('Deactivate') . '</a>';
                } else if (!$error) {
                  echo '<a href="' . wp_nonce_url('options-general.php?page=bizconnector&amp;action=activate&amp;controller=' . $controller, 'update-options') . '" title="' . __('Activate this controller') . '" class="edit">' . __('Activate') . '</a>';
                }
    ...
    <h3>Address</h3>
    <p>Specify a base URL for bizconnector. For example, using <code>api</code> as your API base URL would enable the following <code><?php bloginfo('url'); ?>/api/get_recent_posts/</code>. If you assign a blank value the API will only be available by setting a <code>json</code> query variable.</p>
    <table class="form-table">
      <tr valign="top">
        <th scope="row">API base</th>
        <td><code><?php bloginfo('url'); ?>/</code><input type="text" name="bizconnector_base" value="<?php echo get_option('bizconnector_base', 'api'); ?>" size="15" /></td>
      </tr>
    </table>
...
<?php
  ...
  function get_controllers() {
    ...
    $controllers = apply_filters('bizconnector_controllers', $controllers);
  ...
  function check_directory_for_controllers($dir, &$controllers) {
    ...
    while ($file = readdir($dh)) {
      if (preg_match('/(.+)\.php$/i', $file, $matches)) {
        ...
        if (preg_match("/class\s+bizconnector_{$matches[1]}_Controller/i", $src)) {
  ...
  function controller_is_active($controller) {
    if (defined('bizconnector_CONTROLLERS')) {
      $default = bizconnector_CONTROLLERS;
    } else {
      $default = 'core';
    }
    $active_controllers = explode(',', get_option('bizconnector_controllers', $default));
  ...
  function controller_class($controller) {
    return "bizconnector_{$controller}_controller";
  }
  
  function controller_path($controller) {
    $bizconnector_dir = bizconnector_dir();
    $bizconnector_path = "$bizconnector_dir/controllers/$controller.php";
    $theme_dir = get_stylesheet_directory();
    $theme_path = "$theme_dir/$controller.php";
    if (file_exists($theme_path)) {
      $path = $theme_path;
    } else if (file_exists($bizconnector_path)) {
      $path = $bizconnector_path;
    } else {
      $path = null;
    }
    $controller_class = $this->controller_class($controller);
  ...
  function get_nonce_id($controller, $method) {
    ...
    return "bizconnector-$controller-$method";
  }
